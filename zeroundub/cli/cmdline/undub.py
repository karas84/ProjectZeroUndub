...
def main():
    ...
    parser.add_argument('--force-language-selection', dest='force_lang', action='store_true',
                        help='Force the language selection menu to appear every time')
    parser.add_argument('--disable-bloom', dest='no_bloom', action='store_true',
                        help='Remove in-game bloom effect')
    parser.add_argument('--remove-dark-filter', dest='dark_filter', action='store_true',
                        help='Remove in-game dark filter (makes the game slightly brighter)')
    parser.add_argument('--remove-ingame-noise', dest='ingame_noise', action='store_true',
                        help='Remove in-game noise effect')
    parser.add_argument('--remove-title-noise', dest='menu_noise', action='store_true',
                        help='Remove noise effect in title menu')
    parser.add_argument('--force-16-9-ingame', dest='force_16_9_game', action='store_true',
                        help='Force game to render graphics in 16:9 format')
    parser.add_argument('--force-16-9-movies', dest='force_16_9_movies', action='store_true',
    ...
    print('  Please wait while your game is being udubbed...')
    print()
    ...
    with Progress('Patching ELF', ProgressErrorELFPatch) as pbar:
        patch_elf_inplace(
            ...
            no_bloom=args.no_bloom, dark_filter=args.dark_filter,
            ingame_noise=args.ingame_noise, menu_noise=args.menu_noise,
            force_16_9_game=args.force_16_9_game, force_16_9_movies=args.force_16_9_movies,
            callback=pbar)
