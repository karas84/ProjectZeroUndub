...
class PJZReader:
    ...
    def parse_cd_file_dat(self, elf_path):
        ...
        toc_entry_list = [TOCEntry(f'{entry.group(1)}.{entry.group(2)}', int(entry.group(3)))
    ...
    def validate_toc(toc_entry_list, file_entry_list):
        return toc_entry_list is not None and \
               file_entry_list is not None and \
               len(toc_entry_list) == len(file_entry_list)

    def validate_image_bd_size(self, file_entry_list):
        max_file_entry_position = max([fe.offset + fe.size for fe in file_entry_list])
        image_bd_size = self.adapter.get_img_db_size()
    ...
    def find_entry(self, file_name) -> Union[None, TOCEntry]:
    ...
    def open(self, file_name):
        ...
        with self.adapter.open(self.adapter.img_bd_path) as file_h:
            yield SubFile(file_h, toc_entry.offset, toc_entry.size)

    def prepare_toc_and_entries(self):
        if not all(map(self.adapter.test_file,
                       (self.adapter.elf_path, self.adapter.img_hd_path, self.adapter.img_bd_path))):
        ...
        if not self.validate_toc(toc_entry_list, file_entry_list) or \
                not self.validate_image_bd_size(file_entry_list):
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}[{self.adapter.__class__.__name__}=' \
