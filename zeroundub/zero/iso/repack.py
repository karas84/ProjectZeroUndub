...
class ReaderUndubEntry(AbstractUndubEntry):
    ...
    def __init__(self, reader: PJZReader, toc_entry: TOCEntry,
                 name: str, number: int, new_size=0, japanese=False):
...
class ExternalFileEntry(AbstractUndubEntry):
    ...
    def __init__(self, file: Union[str, bytes, BinaryIO], name: str, number: int, new_size=0):
...
def patch_elf_inplace(
        iso_path: str, force_lang=False,
        no_bloom=False, dark_filter=False, ingame_noise=False, menu_noise=False,
        force_16_9_game=False, force_16_9_movies=False,
        callback=None):
    ...
    if callback:
        callback(2 + int(force_lang) + int(no_bloom) + int(dark_filter) + int(ingame_noise) + int(menu_noise) +
                 int(force_16_9_game) + int(force_16_9_movies))
    
    with open(iso_path, mode='rb+') as iso_fh:
        file_h = SubFile(iso_fh, offset, size)
    ...
    for jp_movie, en_movie in all_matches:
        ...
        with iso_jp.open_file_from_iso(iso_path=jp_movie) as jp_movie_fh:
            with open(iso_undub_path, 'rb+') as iso_undub_fh:
                en_movie_fh = SubFile(iso_undub_fh, offset=offset_undub, size=size_undub)
                pss_mux_from_bytes_io(jp_movie_fh, en_movie_fh)
...
def merge_iso_img_bd_contents(eu_iso_path: str, jp_iso_path: str, out_iso_path: str, replace_sfx=False, callback=None):
    ...
    entries_eu: List[TOCEntry] = [reader_eu.find_entry(name) for name in reader_eu.list_files()]
    entries_jp: List[TOCEntry] = [reader_jp.find_entry(name) for name in reader_jp.list_files()]
    ...
    def filter_jp_entries_common(_pattern, pad=False):
        ...
        for toc_jp in entries_jp:
            if re.match(_pattern, toc_jp.name) and toc_jp.name in file_name_list_eu:
                ...
                _eujp_entries[toc_eu.name] = \
                    ReaderUndubEntry(reader_jp, toc_jp, toc_eu.name, toc_eu.number, new_size=size, japanese=True)
    ...
    undub_entries: List[AbstractUndubEntry] = list()
    ...
    sizes = [entry.new_size for entry in sorted(undub_entries, key=lambda e: e.number)]

    max_img_bd_size = reader_eu.adapter.get_img_db_size()
    found = False
    offsets = list()
    img_bd_size = -1
    ...
    with open(out_iso_path, 'rb+') as iso_fh:
        for n, (entry, offset) in enumerate(zip(undub_entries, offsets)):
            ...
            zero_padding = next_offset - (current_offset + entry.data_size)
